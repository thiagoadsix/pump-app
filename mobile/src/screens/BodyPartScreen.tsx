...
export function BodyPartScreen({
  navigation,
}: NativeStackScreenProps<HomeScreenParamList, 'BodyPartScreen'>) {
  const [selected, setSelected] = useState<string>('back');
  const [exercises, setExercises] = useState<Array<Exercise>>([]);

  useEffect(() => {
    api.get(`local/exercises/body-part/${selected}`).then(result => {
  ...
  return (
    <>
      ...
      <Box display="flex" flex="1" backgroundColor="primary.900" padding="5">
