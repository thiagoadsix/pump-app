...
import {Workout} from '../contracts/interfaces';
import {HomeScreenParamList} from '../routes/app.routes';
import {useAuth} from '../hooks/useAuth';

export function AddExerciseToWorkoutScreen({
  navigation,
  route,
}: NativeStackScreenProps<HomeScreenParamList, 'AddExerciseToWorkoutScreen'>) {
  ...
  useEffect(() => {
    api
      .get(`/local/workouts/user/${user.id}`)
      .then(result => {
  ...
  const handleAddExerciseToWorkoutList = async () => {
    await api.post(`local/workouts/${workoutId}/user/${user.id}/exercise`, {
      sets: [
        {
          id: exercise.id,
          repetitions,
          series,
          weight: exercise.equipment !== 'body weight' ? weight : 0,
        },
      ],
    });
    setShowModal(false);
    ...
    <>
      ...
      <TouchableWithoutFeedback
        ...
        <KeyboardAvoidingView
          ...
          />
          <Text fontSize="24" fontWeight="bold" color="general.900" marginY="2">
            {exercise.name}
          </Text>
          ...
          {showWorkouts ? (
              ...
              <Stack>
                <Pressable
                  onPress={handleSubmit(
                    handleShowModalToAddExerciseToWorkoutList,
                  )}
            ...
            <View flex="1">
              <ScrollView>
                <FormControl
                  isRequired
                  isInvalid={'repetitions' in errors}
                  marginBottom="2">
                  <FormControl.Label>
                    <Text bold color="general.900" fontSize="16">
                      Repetitions
                    </Text>
                  </FormControl.Label>
                  <Controller
          ...
          <Modal isOpen={showModal} onClose={() => setShowModal(false)}>
            <Modal.Content maxWidth="400px">
              ...
              <Modal.Body
                ...
                alignItems="center"
                justifyContent="center">
                <Ionicons name="checkmark-done" size={20} color="#00E9D6" />
                <Text fontSize="14" textAlign="center" color="general.900">
                  Exercise and details added with successful. You'll will be
                  redirected to Body Part screen.
                </Text>
              </Modal.Body>
              <Modal.Footer
                borderTopWidth={2}
                borderTopColor="secondary.900"
                backgroundColor="primary.600"
                alignItems="center"
                justifyContent="center">
                <Button.Group>
