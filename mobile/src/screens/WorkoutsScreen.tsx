...
export function WorkoutsScreen({
  navigation,
}: NativeStackScreenProps<HomeScreenParamList, 'WorkoutsScreen'>) {
  const {workouts, setWorkouts} = useWorkout();
  const {user} = useAuth();

  useEffect(() => {
    api.get(`local/workouts/user/${user.id}`).then(result => {
  ...
  const handleRemovePress = (workoutId: string) => {
    api
      .delete(`/local/workouts/${workoutId}/user/${user.id}`)
      .then(() =>
    ...
    <>
      ...
      <Box display="flex" flex="1" padding="5" backgroundColor="primary.900">
        <Box
          ...
          alignItems="center"
          marginBottom="5">
          <Text color="secondary.900" fontWeight="bold" fontSize="24">
            Workouts
          </Text>
          <Pressable
            height="10"
          ...
          renderItem={({item}) => (
            <Pressable onPress={() => handleWorkoutPress(item)}>
              <Box
                ...
                padding="5"
                marginBottom="4">
                <Text fontWeight="bold" fontSize="18" color="general.900">
                  {item.name}
                </Text>
                <Text fontWeight="normal" fontSize="14" color="general.900">
